# Firstly what is alt_inheritance?
'''
. Since we know about inheritance, Now this is 'alternative inheritance' means alt_inheritance.
Let's learn this in an easy way by the example:
...
# Child class tries to redefine 'age' , Normally this would cause an error.
# But with alt inheritance techniques (like setting `repr=False`), you can avoid issues.
@dataclass(repr=False)  # Alternative inheritance behaviour
class Child(Parent):
    age: float  # Override parent's 'age' (now a float instead of int)
    school: str

# Usage
child = Child(name="Alice", age=12.5, school="XYZ School")
print(child)  # Works without errors due to alt inheritance setup
...
Key Uses of @property:
Convert a method into an attribute (no parentheses needed).
Add validation/logic when getting/setting/deleting an attribute.
Maintain backward compatibility if you later need to add logic to an existing attribute.
'''
"""
